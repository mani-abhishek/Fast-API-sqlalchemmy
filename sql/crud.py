...
def create_batch(db: Session, batch: schemas.BatchBase):
    ...
    school_id = db.query(models.School).filter(models.School.school_name == batch.school_name).first()
    if school_id == None:
...
async def get_topic(db:Session, phone_number :int):
    user = db.query(models.User).filter(models.User.phone_number == phone_number).first()

    if(user.role == "Student"):
        All_Topic = {"Teacher_Topic":[],"School_Topic":"","Own_Student_Topic":"","Global_Topic":"com.hertzai.global","Global_Student_Topic":"com.hertzai.student"}
        student = db.query(models.Student).filter(models.Student.user_id == user.user_id).first()
        batch = db.query(models.Batch).filter(models.Batch.batch_id == student.batch_id).first()
        course = db.query(models.BatchCourseNormalization).filter(models.BatchCourseNormalization.batch_id == batch.batch_id).first()
        teachers = db.query(models.TeacherStudentNorm).filter(models.TeacherStudentNorm.student_id == student.student_id).all()
        
        
        print("Student Logged In")
        print("Standard ID: ",batch.standard_id,"\nCourse ID : ",course.course_id)

        for teacher in teachers:
            All_Topic["Teacher_Topic"].append(f"T{teacher.teacher_id}.Course{course.course_id}.Standard{batch.standard_id}")
        

        All_Topic["Own_Student_Topic"]=f"S.{student.student_id}"
        All_Topic["School_Topic"]= f"IS.{student.school_id}"
        ...
        All_Topic = {"Own_Teacher_Topic":"","School_Topic":"","Global_Topic":"com.hertzai.global","Global_Student_Topic":"com.hertzai.teacher"}
        teacher = db.query(models.Teacher).filter(models.Teacher.user_id == user.user_id).first()
        
        
        All_Topic["Own_Teacher_Topic"] = f"T.{teacher.teacher_id}"

        
        All_Topic["School_Topic"]=f"IT.{teacher.school_id}"
        

        print("All topic \n",All_Topic)

    elif user.role == "School" :
        print("School Logged In")
        All_Topic = {"Own_School_Topic":"","Global_Topic":"com.hertzai.global","Global_Student_Topic":"com.hertzai.school"}
        school = db.query(models.School).filter(models.School.user_id == models.User.user_id).first()

        All_Topic["Own_School_Topic"]=f"S.{school.school_id}"
