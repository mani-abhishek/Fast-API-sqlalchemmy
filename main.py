...
# crossbar topic
@controller.get("/topic/{phone_number}", tags=["Topic"])
async def get_topic(phone_number: int, db:Session = Depends(get_db)):
    requests = await crud.get_topic(db, phone_number = phone_number)
    return requests
